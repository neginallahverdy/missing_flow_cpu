...
def loglik_pos(batch_data, list_type, theta, normalization_params, extra_params=None):
    """Log-likelihood for strictly positive variables using a log-normal model."""
    output = dict()
    epsilon = 1e-3
    ...
    est_mean = theta[:, indx]
    if theta.shape[1] >= 2 * len(indx):
        est_log_var = theta[:, indx + len(indx)]
    else:
        est_log_var = torch.zeros_like(est_mean)
    ...
    est_var = torch.clamp(est_var, epsilon, 1e20)
    est_var = torch.nan_to_num(est_var, nan=epsilon, posinf=1e20, neginf=epsilon)
    est_mean = torch.nan_to_num(est_mean, nan=0.0)
...
def loglik_count(batch_data, list_type, theta, normalization_params, extra_params=None):
    ...
    # Poisson observations must be non-negative integers. Clamp and round to
    # avoid invalid values.
    data = torch.nan_to_num(data, nan=0.0, neginf=0.0)
    ...
    # Replace any invalid values produced by the network
    est_lambda = torch.nan_to_num(est_lambda, nan=epsilon, posinf=1e20, neginf=epsilon)
