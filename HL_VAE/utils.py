    ...
    for i, tpl in enumerate(types_info['set_of_types']):
        ...
        if tpl[0] == 'real':
            ...
            if types_info['conv']:
                ...
                missing_sum = missing_block.sum(dim=0)
                safe_den = torch.where(missing_sum > 0, missing_sum, torch.ones_like(missing_sum))
                data_mean = (observed_data * missing_block).sum(dim=0) / safe_den
                data_var = torch.sum(((observed_data - data_mean) * missing_block)**2, 0) / safe_den
                data_mean[missing_sum == 0] = 0.0
                data_var[missing_sum == 0] = 1.0
            ...
            missing_sum = missing_block.sum(dim=0)
            safe_den = torch.where(missing_sum > 0, missing_sum, torch.ones_like(missing_sum))
            data_mean_log = (observed_data_log * missing_block).sum(dim=0) / safe_den
            data_var_log = torch.sum(((observed_data_log - data_mean_log) * missing_block)**2, 0) / safe_den
            data_mean_log[missing_sum == 0] = 0.0
            data_var_log[missing_sum == 0] = 1.0
