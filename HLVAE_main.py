...
from model_test import HLVAETest , HLVAETest_with_imputation_save
...
if __name__ == "__main__":
    ...
    dataset = CustomWeatherDataset(
        ...
        mask_file=mask_file,
        true_miss_file=true_mask_file,
        types_file=csv_types_file,
        range_file=csv_range_file,
    )
    ...
    if run_tests or generate_images:
        if not all(
            ...
            raise ValueError(
                "Prediction dataset requested but prediction data, labels or mask file is missing"
            )

        prediction_dataset = CustomWeatherDataset(
            root_dir=data_source_path,
            csv_file_data=csv_file_prediction_data,
            csv_file_label=csv_file_prediction_label,
            mask_file=prediction_mask_file,
            true_miss_file=true_prediction_mask_file,
            types_file=csv_types_file,
            range_file=csv_range_file,
        )
    ...
    if generate_images:
        if not all(
            [csv_file_generation_data, csv_file_generation_label, generation_mask_file]
        ):
            raise ValueError(
                "Image generation requested but generation data, labels or mask file is missing"
            )

        generation_dataset = CustomWeatherDataset(
            root_dir=data_source_path,
            csv_file_data=csv_file_generation_data,
            csv_file_label=csv_file_generation_label,
            mask_file=generation_mask_file,
            true_miss_file=true_generation_mask_file,
            types_file=csv_types_file,
            range_file=csv_range_file,
        )
    else:
    ...
    if run_validation:
        if not all(
            [csv_file_validation_data, csv_file_validation_label, validation_mask_file]
        ):
            raise ValueError(
                "Validation requested but validation data, labels or mask file is missing"
            )

        validation_dataset = CustomWeatherDataset(
            root_dir=data_source_path,
    ...
    try:
        ...
        print(f"Failed to load GP model or zt_list: {e}")
    ...
    try:
        ...
        print(f"Could not load m or H: {e}")
    ...
    if epochs > 2 and not early_stopping:
        ...
        try:
            if 'save_path' not in locals() or save_path is None:
                raise NameError("The variable 'save_path' is not defined. Please ensure it is set before saving models.")
            ...
            raise RuntimeError(f"Failed to save model files: {e}")
    ...
    ###########################################################################################################################################################################
    # ADD THE IMPUTATION SAVING CODE HERE:
    print("\n=== SAVING IMPUTATION RESULTS ===")
    from model_test import HLVAETest_with_imputation_save

    # Save imputation results for test dataset
    test_imputation_save_path = os.path.join('results', "test_ground_truth_vs_imputed.csv")
    test_comparison_df = HLVAETest_with_imputation_save(
        test_dataset, 
        nnet_model, 
        test_imputation_save_path,
        test=prediction_flag,  # Uses the same flag as your existing test
        id_covariate=id_covariate,
        T=T,
        training_indexes=dataset.label_source.iloc[:,-1]
    )

    # Optionally save imputation results for validation dataset too
    if run_validation and validation_dataset is not None:
        validation_imputation_save_path = os.path.join('results', "validation_ground_truth_vs_imputed.csv")
        validation_comparison_df = HLVAETest_with_imputation_save(
            validation_dataset, 
            nnet_model, 
            validation_imputation_save_path,
            test=False,  # Evaluate all validation data
            id_covariate=id_covariate,
            T=T,
            training_indexes=dataset.label_source.iloc[:,-1]
        )   
