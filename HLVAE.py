...
class HLVAE(nn.Module):
    ...
    def __init__(self, dims=None, types_info=None, n_variables=None, vy_init=[1., .5], vy_fixed=False, logvar_network=False, conv=True,
                 x_dim=None, h_dim_e=None, z_dim=None, h_dim_d=None, y_dim=None):
        """Initialize the HLVAE network.

        Parameters
        ----------
        dims : list or tuple, optional
            Legacy positional list ``[x_dim, h_dim_e, z_dim, h_dim_d, y_dim]``.
            If not provided, the individual ``x_dim`` ... ``y_dim`` arguments are
            used instead.  This keeps backwards compatibility with older
            training scripts while also allowing explicit named arguments as in
            ``vae_impute.py``.
        types_info : dict
            Dictionary describing the data types.
        n_variables : int
            Number of output variables.
        vy_init : list of float, optional
            Initial variance for real and positive observations.
        vy_fixed : bool, optional
            Whether observation variances are fixed.
        logvar_network : bool, optional
            If True, observation variance is predicted by a neural network.
        conv : bool, optional
            Use the convolutional variant of the architecture.
        x_dim, h_dim_e, z_dim, h_dim_d, y_dim : optional
            Named arguments providing the same values as ``dims``.
        """
        super(HLVAE, self).__init__()

        if dims is None:
            provided = [x_dim, h_dim_e, z_dim, h_dim_d, y_dim]
            if any(v is None for v in provided):
                raise TypeError(
                    "Either `dims` or all of `x_dim`, `h_dim_e`, `z_dim`, `h_dim_d`, and `y_dim` must be provided"
                )
            dims = [x_dim, h_dim_e, z_dim, h_dim_d, y_dim]
