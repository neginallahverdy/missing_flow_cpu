...
class Likelihoods(nn.Module):
    def __init__(self, latent_dim, noise, constrain=True):
        self.kernel_list[0].lengthscale = torch.nn.Parameter(torch.tensor([math.log(5.0)]))   # 5 روز
        # طول‌مقیاس کرنل دوم: بلند
        self.kernel_list[1].lengthscale = torch.nn.Parameter(torch.tensor([math.log(30.0)]))  # 30 روز
